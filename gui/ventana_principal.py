...
class AppVeterinaria(tk.Tk):
    ...
    def refrescar(self, items=None):
        ...
        for idx, c in enumerate(datos):
            ...
            self.tabla.insert("", "end", iid=str(c.id_cita),
                              values=(c.id_cita, nombre_mascota, nombre_dueno,
                                      c.motivo, c.fecha_hora.strftime("%Y-%m-%d"), c.fecha_hora.strftime("%H:%M"),
                                      c.id_veterinario),
                              tags=(tag,))
        sel_txt = f" · Seleccionada: #{self.seleccion_id}" if self.seleccion_id else ""
        self.var_contador.set(f"{len(datos)} mostrada(s) · Total: {len(self.sistema.listar_citas())}{sel_txt}")
    ...
    def refrescar_contador(self):
        ...
        sel_txt = f" · Seleccionada: #{self.seleccion_id}" if self.seleccion_id else ""
        self.var_contador.set(f"{n} mostrada(s) · Total: {len(self.sistema.listar_citas())}{sel_txt}")
    ...
    def _formulario(self, cita: Cita):
        ...
        tb.pack(fill="x")
        tk.Label(tb, text=("Nueva cita" if cita is None else f"Editar cita #{cita.id_cita}"),
        ...
        dueno_obj = self.sistema.buscar_dueno(mascota_obj.id_dueno) if mascota_obj else None
        ...
        campos = [
            ...
            ("Fecha (YYYY-MM-DD)", "fecha", cita.fecha_hora.strftime("%Y-%m-%d") if cita else ""),
        ...
        for i, (label, key, val) in enumerate(campos):
            tk.Label(cont, text=label, bg=WINDOW, fg=TEXT, font=FONT).grid(row=i, column=0, sticky="w", pady=6)
            v = tk.StringVar(value=val)
            vars_[key] = v
            e = tk.Entry(cont, textvariable=v, font=FONT, width=32, relief="solid", bd=1,
        ...
        FlatButton(botones, "Cancelar", GHOST, GHOST_FG, GHOST_HOVER, top.destroy).pack(side="left", padx=(0, 6))

        def guardar():
            datos = {k: v.get().strip() for k, v in vars_.items()}
            if not all(datos.values()):
                messagebox.showwarning("Campos", "Completa todos los campos.", parent=top)
                return
            try:
                dt_fecha = datetime.strptime(datos["fecha"], "%Y-%m-%d")
                dt_hora = datetime.strptime(datos["hora"], "%H:%M")
                fecha_hora_completa = dt_fecha.replace(hour=dt_hora.hour, minute=dt_hora.minute)
            except ValueError:
                messagebox.showwarning("Formato", "Fecha: YYYY-MM-DD · Hora: HH:MM", parent=top)
                return
            ...
            if cita is None:
                ...
                nueva_cita = Cita(cid, fecha_hora_completa, datos["motivo"], "AGENDADA", mascota.id_mascota, datos["veterinario"])
                exito = self.sistema.agendar_cita(nueva_cita)
                if exito:
                    self.var_estado.set(f"Cita #{cid} creada.")
                else:
                    messagebox.showwarning("Error", "No se pudo agendar la cita (conflicto de horario).", parent=top)
                    return
            else:
                exito = self.sistema.modificar_cita(cita.id_cita, motivo=datos["motivo"], fecha_hora=fecha_hora_completa, id_veterinario=datos["veterinario"])
                if exito:
        ...
        FlatButton(botones, "Guardar", PRIMARY, "white", PRIMARY_HOVER, guardar).pack(side="left")
