...
class AppVeterinaria(tk.Tk):
    ...
    def refrescar(self, items=None):
        ...
        self.var_contador.set(
            f"{len(datos)} mostrada(s) · Total: {len(self.citas)}{sel_txt}"
        )
    ...
    def eliminar_sel(self):
        ...
        if messagebox.askyesno(
            "Eliminar", f"¿Eliminar la cita #{c['id']} de {c['mascota']}?"
        ):
            cid = c["id"]
            del self.citas[cid]
            self.seleccion_id = None
    ...
    def _formulario(self, cita):
        ...
        tk.Label(
            tb,
            text=("Nueva cita" if cita is None else f"Editar cita #{cita['id']}"),
            bg=TITLEBAR,
            fg=TITLEBAR_FG,
            font=FONT_SM,
            padx=12,
