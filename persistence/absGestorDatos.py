...
class GestorDatos:
    ...
    def cargar_citas_csv(self, archivo: str | Path) -> list[dict[str, Any]]:
        return self._leer_csv(Path(archivo), columnas_esperadas=[
            "id_cita", "fecha_hora", "motivo", "estado", "id_mascota", "id_veterinario"
        ])
    ...
    def guardar_citas_csv(self, archivo: str | Path, citas: Iterable[Any]) -> None:
        ...
        self._escribir_csv(Path(archivo), filas, columnas=[
            "id_cita", "fecha_hora", "motivo", "estado", "id_mascota", "id_veterinario"
        ])


    def cargar_duenos(self) -> list[dict[str, Any]]:
        return self._leer_csv(self.archivo_duenos, columnas_esperadas=[
            "id_persona", "nombre", "telefono", "email", "direccion"
        ])

    def guardar_duenos(self, duenos: Iterable[Any]) -> None:
        filas = [self._obj_a_dict(d) for d in duenos]
        self._escribir_csv(self.archivo_duenos, filas, columnas=[
            "id_persona", "nombre", "telefono", "email", "direccion"
        ])

    def cargar_mascotas(self) -> list[dict[str, Any]]:
        filas = self._leer_csv(self.archivo_mascotas, columnas_esperadas=[
            "id_mascota", "nombre", "especie", "raza", "edad", "sexo", "peso", "id_dueno"
        ])
        for f in filas:
    ...
    def guardar_mascotas(self, mascotas: Iterable[Any]) -> None:
        ...
        self._escribir_csv(self.archivo_mascotas, filas, columnas=[
            "id_mascota", "nombre", "especie", "raza", "edad", "sexo", "peso", "id_dueno"
        ])

    def cargar_citas(self) -> list[dict[str, Any]]:
        filas = self._leer_csv(self.archivo_citas, columnas_esperadas=[
            "id_cita", "fecha_hora", "motivo", "estado", "id_mascota", "id_veterinario"
        ])
        for f in filas:
            if "fecha_hora" in f:
    ...
    def guardar_citas(self, citas: Iterable[Any]) -> None:
        ...
        for f in filas:
            if "fecha_hora" in f:
                f["fecha_hora"] = self._normalizar_fecha_hora(f["fecha_hora"])
        self._escribir_csv(self.archivo_citas, filas, columnas=[
            "id_cita", "fecha_hora", "motivo", "estado", "id_mascota", "id_veterinario"
        ])


    def _leer_csv(self, ruta: Path, columnas_esperadas: list[str]) -> list[dict[str, Any]]:
        if not ruta.exists():
            return []
    ...
    def _escribir_csv(self, ruta: Path, filas: list[dict[str, Any]], columnas: list[str]) -> None:
        if not filas:
