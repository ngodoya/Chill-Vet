...
class ValidadorDatos:
    ...
    def validar_id_unico(id_valor: str, coleccion: list) -> bool:
        ...
        for item in coleccion:
            ...
            val_id = (
                getattr(item, "id_persona", None)
                or getattr(item, "id_mascota", None)
                or getattr(item, "id_veterinario", None)
                or getattr(item, "id_cita", None)
            )
    ...
    def validar_referencias(
        id_mascota: str, id_veterinario: str, mascotas: list, veterinarios: list
    ) -> bool:
